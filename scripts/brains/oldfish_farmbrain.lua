...
local function PickBerriesAction(inst)
    ...
    if target then
        ...
        elseif target.prefab == "rock_avocado_bush" and target.components.growable ~= nil then
            if target.components.growable.stage == 3 and inst.components.container:GetItemInSlot(3) ~= nil and inst.components.container:GetItemInSlot(3).prefab == "pickaxe" then
        ...
        inst.components.container:GetItemInSlot(1) ~= nil and inst.components.container:GetItemInSlot(1).prefab == "shovel" and not target:HasTag("fire") then
            return BufferedAction(inst, target, ACTIONS.PICK)
        elseif target.prefab ~= "berrybush" and target.prefab ~= "berrybush2" and target.prefab ~= "berrybush_juicy" and 
        target.prefab ~= "grass" and target.prefab ~= "sapling" and target.prefab ~= "sapling_moon" and target.components.pickable and not target:HasTag("fire") then
            return BufferedAction(inst, target, ACTIONS.PICK)
        end
    end
end
