...
function FireDetectorPlus:SetOnFindFireFn(fn)
    self.onfindfire = fn
end

function FireDetectorPlus:SetOnBeginEmergencyFn(fn)
    self.onbeginemergency = fn
end

function FireDetectorPlus:SetOnEndEmergencyFn(fn)
    self.onendemergency = fn
end

function FireDetectorPlus:SetOnBeginWarningFn(fn)
    self.onbeginwarning = fn
end

function FireDetectorPlus:SetOnUpdateWarningFn(fn)
    self.onupdatewarning = fn
end

function FireDetectorPlus:SetOnEndWarningFn(fn)
...
function FireDetectorPlus:OnRemoveFromEntity()
...
function FireDetectorPlus:Activate(randomizedStartTime)
    Cancel(self.inst, self)
...
function FireDetectorPlus:ActivateEmergencyMode(randomizedStartTime)
    Cancel(self.inst, self)
    self.emergencyBurnt = {}
...
function FireDetectorPlus:IsEmergency()
    return self.emergency
end

function FireDetectorPlus:GetEmergencyLevel()
    return self.emergencyLevel
end

function FireDetectorPlus:GetMaxEmergencyLevel()
    return self.emergencyLevelMax
end

function FireDetectorPlus:ResetEmergencyCooldown()
    if self.emergencyBurnt ~= nil then
        local t = GetTime()
...
function FireDetectorPlus:RaiseEmergencyLevel(numlevels)
    if self.emergencyBurnt ~= nil then
...
function FireDetectorPlus:LowerEmergencyLevel(numlevels)
    if self.emergencyBurnt ~= nil then
...
function FireDetectorPlus:Deactivate()
...
function FireDetectorPlus:DetectFire()
    if self.detectTask ~= nil then
        if self.emergency then
...
function FireDetectorPlus:GetDebugString()
